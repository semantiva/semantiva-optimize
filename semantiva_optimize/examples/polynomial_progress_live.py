...
class FallbackLeastSquaresObjective:
    ...
    def objective(self, theta):
        ...
        print(
            f"Function call {self.calls}: theta={[round(t, 3) for t in theta]}, f={residual_sum:.6f}"
        )
        return residual_sum
...
class SlowProgressObserver:
    ...
    def on_step(self, e):
        print(
            f"{self.name}: Step {e.iter} - x={[round(xi, 3) for xi in e.x]}, f={e.f:.6f}, is_best={e.is_best}"
        )
...
print(
    "You'll see live matplotlib windows showing current (dashed) and best (solid) fits."
)
print()
...
objective = LeastSquaresObjective(
    family=PolynomialFamily(degree=degree), x_data=x_data, y_data=y_data
)
