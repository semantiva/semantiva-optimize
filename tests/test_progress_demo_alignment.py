...
def test_degree_alignment_and_png(tmp_path, monkeypatch):
    ...
    assert len(x0) == degree + 1, f"x0 length ({len(x0)}) must equal degree+1 ({degree+1})"
    ...
    iter_pngs = [f for f in tmp_path.iterdir() if "_iter" in f.name and f.name.endswith(".png")]
